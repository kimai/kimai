...
export default class KimaiCalendar {
  ...
  constructor(kimai, element, options) {
    ...
    let calendarOptions = {
      ...
      slotMaxTime:
        this.options["timeframeEnd"] === "23:59"
          ? "24:00:00"
          : this.options["timeframeEnd"] + ":59",
      ...
      datesSet: (dateInfo) => {
        document.dispatchEvent(
          new CustomEvent("kimai.calendar.changeDate", {
            detail: {
      ...
      eventMouseEnter: (mouseEnterInfo) => {
        ...
        const popoverTitle =
          DATES.getFormattedDate(event.start) +
          " | " +
          DATES.formatTime(event.start) +
          " - " +
          (event.end ? DATES.formatTime(event.end) : "");
        ...
        arg.el.addEventListener("contextmenu", (jsEvent) => {
          ...
          if (!event.allDay) {
            ...
            API.get(
              ...
              {},
              (result) => {
                const contextMenu = new KimaiContextMenu(
                  "calendar_contextMenu"
                );
                contextMenu.createFromApi(jsEvent, result);
              },
              (e) => {
                console.log("Failed to load actions for context menu", e);
              }
            );
          }
        });
      ...
      eventsSet: (events) => {
        this._renderDayAndWeekSum(
          this.getCalendar().getCurrentData().viewSpec.type,
          events
        );
      },
    };
    ...
    if (
      !this.hasPermission("punch") &&
      this.hasPermission("create") &&
      this.options.dragdrop !== undefined
    ) {
      const draggableList = [].slice.call(
        document.querySelectorAll(this.options.dragdrop.container)
      );
      ...
      calendarOptions = {
        ...
        ...{
          ...
          drop: (dropInfo) => {
            ...
            let end = DATES.addHumanDuration(
              begin,
            ...
            if (source.dataset.method === "PATCH") {
              API.patch(apiUrl, JSON.stringify(data), (result) => {
              ...
              API.post(apiUrl, JSON.stringify(data), (result) => {
                const newItem = this.convertSourceForCalendar(result);
                this.getCalendar().addEvent(newItem, true);
                ALERT.success("action.update.success");
              });
      ...
      calendarOptions = {
        ...
        ...{
          ...
          select: (selectionInfo) => {
            ...
            const createUrl = this.options.url.create(
      ...
      calendarOptions = {
        ...
        ...{
          eventClick: (eventClickInfo) => {
            ...
            if (
              !event.extendedProps.exported ||
              this.hasPermission("edit_exported")
            ) {
              MODAL.openUrlInModal(
                this.options.url.edit(event.id),
                (reason) => {
        ...
        calendarOptions = {
          ...
          ...{
            ...
            eventDurationEditable:
              this.hasPermission("edit_end") ||
              this.hasPermission("edit_duration"),
            eventDragStart: (info) => {
    ...
    for (const source of this.options["eventSources"]) {
      ...
      if (source.type === "timesheet") {
        ...
        console.log(
          "Unknown source type given, skipping to load events from: " +
            source.id
        );
        continue;
      }
  ...
  convertSourceForCalendar(apiItem) {
    ...
    if (apiItem.end === null) {
      ...
      title = title.replace(
        "{duration}",
        DATES.formatDuration(apiItem.duration)
      );
    }
    ...
    return (
      `
            <div class="calendar-entry">
                <ul>
                    <li>` +
      this.options["translations"]["customer"] +
      `: ` +
      escaper.escapeForHtml(eventObj.customer) +
      `</li>
                    <li>` +
      this.options["translations"]["project"] +
      `: ` +
      escaper.escapeForHtml(eventObj.project) +
      `</li>
                    <li>` +
      this.options["translations"]["activity"] +
      `: ` +
      escaper.escapeForHtml(eventObj.activity) +
      `</li>
                </ul>` +
      ...
      (eventObj.description
        ? "<div>" + escaper.escapeForHtml(eventObj.description) + "</div>"
        : "") +
      tags +
      `
            </div>`
  ...
  _renderDayAndWeekSum(view, events) {
    ...
    if (view === "timeGridWeek") {
      ...
      document
        .querySelectorAll(`th.fc-col-header-cell[data-date]`)
        .forEach((cell) => {
    ...
    for (const dateValue in durations) {
      ...
      if (view === "timeGridWeek") {
        ...
        const headerCells = document.querySelectorAll(
          `th.fc-col-header-cell[data-date="${dateValue}"]`
        );
