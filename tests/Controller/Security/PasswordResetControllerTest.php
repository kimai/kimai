...
class PasswordResetControllerTest extends ControllerBaseTest
{
    ...
    public function testResetRequestPageIsRendered()
    {
        ...
        $this->assertStringContainsString('<form action="/en/resetting/send-email" method="POST" class="resetting_request">', $content);
        ...
        $form = $client->getCrawler()->filter('form.resetting_request')->form();
        $client->submit($form, [
            'username' => 'john_user',
        ]);
