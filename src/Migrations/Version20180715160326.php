...
final class Version20180715160326 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $users = $schema->getTable($this->getTableName('users'));

        // delete all existing indexes
        $indexesOld = $users->getIndexes();
        ...
        , username VARCHAR(180) NOT NULL, username_canonical VARCHAR(180) NOT NULL, email VARCHAR(180) NOT NULL, email_canonical VARCHAR(180) NOT NULL, salt VARCHAR(255) DEFAULT NULL, last_login DATETIME DEFAULT NULL, confirmation_token VARCHAR(180) DEFAULT NULL, password_requested_at DATETIME DEFAULT NULL, PRIMARY KEY(id))');
            $this->addSql('INSERT INTO kimai2_users (id, username, username_canonical, email, email_canonical, password, alias, enabled, registration_date, title, avatar, roles) SELECT id, name, name, mail, mail, password, alias, active, registration_date, title, avatar, roles FROM __temp__kimai2_users');
    ...
    public function down(Schema $schema): void
    {
        ...
        if ($platform === 'sqlite') {
            ...
            $this->addSql('INSERT INTO kimai2_users (id, name, mail, active, password, roles, alias, registration_date, title, avatar) SELECT id, username, email, enabled, password, roles, alias, registration_date, title, avatar FROM __temp__kimai2_users');
            $this->addSql('DROP TABLE __temp__kimai2_users');
        } elseif ($platform === 'mysql') {
        ...
        foreach ($this->indexesOld as $index) {
            $users->addIndex($index->getColumns(), $index->getName(), $index->getFlags(), $index->getOptions());
        }
    }
}
